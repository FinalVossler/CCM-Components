...
import { doNothing, getEditorContent } from "../utils";
      ...
      return {
        ...
        events: {
          ...
          keydown: function (e) {
            ...
            if (isALowerCaseCharacterKeyCode(e.keyCode)) {
              const selection = froalaEditorRef.current.editor.selection.get();

              const first =
                document.getSelection().anchorNode.textContent[
                  //@ts-ignore
                  document.getSelection().baseOffset - 2
                ];

              const second =
                document.getSelection().anchorNode.textContent[
                  //@ts-ignore
                  document.getSelection().baseOffset - 1
                ];

              if (
                (!first && !second) ||
                (first?.charCodeAt(0) === 46 && second?.charCodeAt(0) === 160)
              ) {
                const nextCharacter = String.fromCharCode(e.keyCode || e.which);

                const nextCharacterUpperCased = nextCharacter.toUpperCase();

                froalaEditorRef.current.editor.html.insert(
                  nextCharacterUpperCased,
                  null,
                  null,
                  selection
                );
              ...
              if (previousCharacter === "-") {
                ...
                if (window.getSelection().rangeCount > 0) {
                  try {
                    ...
                    let froalaParent =
                      parentContainerDivRef.current.querySelector(
                        ".fr-element"
                      );
          ...
          initialized: async function (e, editor) {
            ...
            this.events.on(
              ...
              (e) => {
                ...
                if (
                  files.some((f) => f.type.indexOf("image") !== -1) &&
                  [...e.originalEvent.dataTransfer.items].every(
                    (el) => el.kind === "file"
                  )
                ) {
                  e.preventDefault();
          ...
          "commands.after": (cmd) => {
            if (cmd === "fullscreen" && toggleEditorFullScreen) {
              toggleEditorFullScreen();
            }
          },
        },
