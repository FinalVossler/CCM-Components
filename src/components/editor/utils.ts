...
export const getSelectionContainer = () => {
  ...
  const selectionStartContainer =
    getSelection().getRangeAt(0).startContainer.parentNode;
  const selectionEndContainer =
    getSelection().getRangeAt(0).endContainer.parentNode;

  if (
    selectionStartContainer.childElementCount <
    selectionEndContainer.childElementCount
  ) {
